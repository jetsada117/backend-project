...
async def get_user_by_id(db: AsyncSession, user_id: int) -> UserResponse:
    sql = text(
        "SELECT id, email, first_name, last_name, hashed_password FROM users WHERE id = :id"
    )
    param = {"id": user_id}
...
async def get_user_by_email(db: AsyncSession, email: str) -> UserResponse:
    sql = text(
        "SELECT id, email, first_name, last_name, hashed_password FROM users WHERE email = :email"
    )
    param = {"email": email}
    result = await db.execute(sql, param)
